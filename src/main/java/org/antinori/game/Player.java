...
public class Player {
	...
	public LifeTile removeLifeTile() {
		if (lifeTiles.size() == 0) return null;
	...
	public void setCareer(CareerCard career) {
		this.career = career;
		if (this.career != null) this.salary = career.getBase_salary();
	}
	
	public int addPayRaise(int amount) {
		salary = salary + amount;
		if (salary > career.getMax_salary()) salary = career.getMax_salary();
		return salary;
	}
	...
	public String getStw_card_1() {
		try {
			if (computerPlayer) return "Hidden";
