...
//					popup.showDialog(getRootPane());
	...
	public void update(GameContainer container, StateBasedGame game, int delta) throws SlickException {
		...
		if (spinWheel) {
			...
			if (wheelSpeed > 0) ang += delta * wheelSpeed;
			wheelSpeed -= 0.0001f;
			...
			if (wheelSpeed < 0) {
				...
				while(true) {
					if (remainder<=36*1) {wheelRoll = 10;break;}
					if (remainder<=36*2) {wheelRoll = 9;break;}
					if (remainder<=36*3) {wheelRoll = 8;break;}
					if (remainder<=36*4) {wheelRoll = 7;break;}
					if (remainder<=36*5) {wheelRoll = 6;break;}
					if (remainder<=36*6) {wheelRoll = 5;break;}
					if (remainder<=36*7) {wheelRoll = 4;break;}
					if (remainder<=36*8) {wheelRoll = 3;break;}
					if (remainder<=36*9) {wheelRoll = 2;break;}
					if (remainder<=36*10) {wheelRoll = 1;break;}
					break;
				}
			} else {
		...
		if (container.getInput().isKeyPressed(Input.KEY_SPACE) || container.getInput().isMouseButtonDown(Input.MOUSE_LEFT_BUTTON)) {
			if (dismissalMessageDisplayed != null) dismissalMessageDisplayed = null;
		}
	...
	public void drawLabelsOnTiles(Graphics g) {
		...
		for (int i = 0; i < map.getWidth(); i++) {
			for (int j = 0; j < map.getHeight(); j++) {
				...
				if (tile == null || tile.getDescription().length() < 1) continue;
	...
	public void drawPlayerPanel(GameContainer container, StateBasedGame game, Graphics g) {
		...
		for (int i=0;i<num_players;i++) {
			...
			if (p.getActor() != currentPlayer) color = OFF;
