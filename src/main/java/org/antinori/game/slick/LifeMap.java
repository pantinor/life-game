...
public class LifeMap extends TiledMapPlus implements TileBasedMap {

	public final Location[][] nodes;
	...
	public LifeMap(String ref) throws SlickException {
		super(ref);

		int width = this.getWidth();
		int height = this.getHeight();

		nodes = new Location[width][height];
		...
		mesh = builder.build(this, false);
		// mesh will merge all the spaces/tiles if it thinks they are all edges
		// aligned into one big space
		System.out.println("mesh space count is " + mesh.getSpaceCount());

	}

	public NavPath getPath(int sx, int sy, int tx, int ty) {
		return mesh.findPath(sx, sy, tx, ty, false);
	}
