...
public class PlayerSelectPanel extends Widget {
    ...
    public PlayerSelectPanel(int numSlotsX, int numSlotsY, String[] icon_names, boolean othersPanel, String[] player_names) {
        ...
        for (int i = 0; i < slot.length; i++) {
            ...
            add(slot[i]);
            if (!othersPanel) {
                slot[i].setModel(new OptionBooleanModel(optionModel, i + 1));
            }
        }
    ...
    public ActorType.Type getSelectedPlayerIcon() {
        ...
        for (int i = 0; i < icon_names.length; i++) {
            if (slot[i].isActive()) {
                if (icon_names[i].equals("girl")) {
                    type = ActorType.Type.PLAYER1;
                }
                if (icon_names[i].equals("arno")) {
                    type = ActorType.Type.PLAYER2;
                }
                if (icon_names[i].equals("builder")) {
                    type = ActorType.Type.PLAYER3;
                }
                if (icon_names[i].equals("lady")) {
                    type = ActorType.Type.PLAYER4;
                }
                if (icon_names[i].equals("santa")) {
                    type = ActorType.Type.PLAYER5;
                }
                if (icon_names[i].equals("doctor")) {
                    type = ActorType.Type.PLAYER6;
                }
                if (icon_names[i].equals("pirate")) {
                    type = ActorType.Type.PLAYER7;
                }
                if (icon_names[i].equals("boy")) {
                    type = ActorType.Type.PLAYER8;
                }
            }
        }
        return type;
